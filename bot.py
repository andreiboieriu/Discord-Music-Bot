...
song_list = ["STARGAZING.mp3",
             "SKELETONS.mp3",
             "5% TINT.mp3",
             "CAN'T SAY.mp3",
             "HOUSTONFORNICATION.mp3"]

@bot.command(brief = 'Play <song_name> from song_list')
async def play(ctx, song_name: str):
    voice_channel = ctx.message.author.voice
    if not voice_channel :
        await ctx.send("Connect to a voice channel first")
    if song_name not in song_list :
        await ctx.send(song_name + " is not available")
    else :
        voice_channel = voice_channel.channel
        await voice_channel.connect()
        voice_channel = ctx.message.guild.voice_client
        await ctx.send("Playing: " + song_name)
...
@bot.command(brief = 'Disconnect from the current voice channel')
async def scram(ctx):
